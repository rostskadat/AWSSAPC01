...
class OpenIDRequestHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        try:
            ...
            self.end_headers()
            self.wfile.write(cgitb.html(sys.exc_info().encode('utf-8'), context=10))
    ...
    def renderSREG(self, sreg_data):
        if not sreg_data:
            self.wfile.write(
                '<div class="alert">No registration data was returned</div>'.encode('utf-8'))
        else:
            ...
            for k, v in sreg_list:
                ...
                self.wfile.write(
                    ('<tr%s><td>%s</td><td>%s</td></tr>' % (odd, field_name, value)).encode('utf-8'))
                if odd:
                    odd = ''
                else:
                    odd = ' class="odd"'
    ...
    def renderPAPE(self, pape_data):
        if not pape_data:
            self.wfile.write(
                '<div class="alert">No PAPE data was returned</div>'.encode('utf-8'))
        else:
            self.wfile.write('<div class="alert">Effective Auth Policies<ul>'.encode('utf-8'))

            for policy_uri in pape_data.auth_policies:
                self.wfile.write(('<li><tt>%s</tt></li>' % (cgi.escape(policy_uri),)).encode('utf-8'))

            if not pape_data.auth_policies:
                self.wfile.write('<li>No policies were applied.</li>'.encode('utf-8'))

            self.wfile.write('</ul></div>'.encode('utf-8'))
...
''' % (quoteattr(self.buildURL('verify')), quoteattr(form_contents))).encode('utf-8'))
