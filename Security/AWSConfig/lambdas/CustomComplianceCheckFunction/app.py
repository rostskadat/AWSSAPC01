...
APPLICABLE_RESOURCES = [VOLUME_TYPE, INSTANCE_TYPE]
...
class EC2_TAG_VOLUMES(ConfigRule):
    ...
    def evaluate_change(self, event, client_factory, configuration_item, valid_rule_parameters):
        ...
        if resource_type == VOLUME_TYPE:
            attachements = configuration_item.get(
                "configuration").get("attachments")
            if not attachements:
                return [Evaluation(ComplianceType.NOT_APPLICABLE)]
            instance_id = attachements[0].get("instanceId")
            ...
            tags_to_apply = compare_instance_tags_to_volume_tags(
                instance_tags, volume_tags)
        ...
        if resource_type == INSTANCE_TYPE:
            ...
            block_device_mappings = configuration_item.get(
                "configuration").get("blockDeviceMappings")
            all_volumes = get_volumes_attached_to_instance(
            ...
            for volume in all_volumes:
                ...
                if not tags_to_apply:
                    evaluations.append(Evaluation(
                        ComplianceType.COMPLIANT, volume_id, VOLUME_TYPE))
                # Scenario:2 EBS Volume Tags does not include the same tags as EC2 Instance its attached to.
                if tags_to_apply:
                    annotation = "EBS Volume needs to have EC2 Instance Tags applied."
                    evaluations.append(
                        Evaluation(ComplianceType.NON_COMPLIANT,
                                   volume_id, VOLUME_TYPE, annotation)
                    )
            return evaluations
...
def lambda_handler(event, context):
    return Evaluator(EC2_TAG_VOLUMES(), APPLICABLE_RESOURCES).handle(event, context)
