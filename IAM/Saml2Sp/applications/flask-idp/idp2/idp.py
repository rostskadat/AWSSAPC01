...
consoleHandler = logging.StreamHandler(sys.stdout) ; consoleHandler.setFormatter(logFormatter) ; logger.addHandler(consoleHandler)
fileHandler = logging.FileHandler(os.path.abspath(os.path.join(os.path.dirname(__file__), "stdout.log"))) ; fileHandler.setFormatter(logFormatter) ; logger.addHandler(fileHandler)
...
def do_health(environ, start_response, _):
    resp = Response("OK")
    return resp(environ, start_response)
...
if __name__ == "__main__":
    ...
    AUTHN_BROKER.add(
        authn_context_class_ref(PASSWORD), username_password_authn, 10, CONFIG.IDP_URL_BASE
    )
    AUTHN_BROKER.add(authn_context_class_ref(UNSPECIFIED), "", 0, CONFIG.IDP_URL_BASE)
    ...
    SRV = WSGIServer(('0.0.0.0', PORT), application)
    ...
    logger.info("Server starting. IDP listening on %s:%s%s", HOST, PORT, _https)
    try:
        SRV.start()
