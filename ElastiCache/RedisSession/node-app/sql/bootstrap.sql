...
CREATE DEFINER=`root`@`%` FUNCTION `LIPSUM`(p_max_words SMALLINT
                                           ,p_min_words SMALLINT
                                           ,p_start_with_lipsum TINYINT(1)) RETURNS VARCHAR(10000)
    NO SQL
    BEGIN
    /**
    ...
    * %version 1.0
    * Example usage:
    * SELECT LIPSUM(5,NULL,NULL) AS fiveWordsExactly;
    * SELECT LIPSUM(10,5,0) AS five-tenWords;
    * SELECT LIPSUM(50,10,1) AS startWithLorumIpsum;
...
SELECT
    RANDSTRING(32), LIPSUM(10, NULL, NULL), 'image.png'
FROM (
    SELECT 
        1 AS n --1 + a.N AS + b.N + ... n
    FROM
